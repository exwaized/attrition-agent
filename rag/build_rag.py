...
# --- Chunking — pure, testable, no I/O ---
def chunk_text(text, chunk_size=300, overlap=50):
    """
    Splits text into overlapping chunks.

    Raises ValueError if overlap >= chunk_size — with that
    combination, start += (chunk_size - overlap) adds zero or a
    negative number on every iteration, so start never advances and
    the loop never terminates. The current callers always pass safe
    values (300/50), but nothing previously stopped a future call
    with bad parameters from hanging forever instead of failing
    loudly — confirmed by actually triggering it under a timeout.
    """
    if overlap >= chunk_size:
        raise ValueError(
            f"overlap ({overlap}) must be smaller than chunk_size "
            f"({chunk_size}), or chunking never advances and hangs forever"
        )
    ...
    while start < len(text):
        ...
        if len(chunk) > 30:
            chunks.append(chunk)
        # Always move forward by (chunk_size - overlap) — guaranteed
        # positive now that overlap < chunk_size is enforced above.
