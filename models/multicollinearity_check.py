# ============================================================
# multicollinearity_check.py — VIF + L1 + Correlation Analysis
# ============================================================
# PURPOSE: Detect collinear features before training
# FLOW: features.csv → VIF scores → L1 selection → drop list
# RUN THIS BEFORE train.py — confirms feature set is clean
...
# VIF and L1 both need scaled features
...
# ============================================================
# |r| > 0.8 = high correlation = multicollinearity risk
# |r| > 0.9 = very high = definitely drop one
# ============================================================
    ...
    for j in range(i+1, len(core_features)):
        ...
        if abs(r) > 0.7:
            high_corr_pairs.append({
                "feature_1": core_features[i],
                "feature_2": core_features[j],
                "r":         round(r, 3),
                "abs_r":     round(abs(r), 3)
            })
...
# ============================================================
# L1 zeroes out redundant features automatically
# If two features are collinear, Lasso keeps one, zeroes other
# Features zeroed = redundant or weak
# ============================================================
...
corr_flagged = set()
if not high_corr_df.empty:
    for _, row in high_corr_df[high_corr_df["abs_r"] > 0.85].iterrows():
...
# Protect interaction terms from dropping
interaction_terms = {f for f in feature_cols if f.startswith("interact_")}

drop_recommended = (
    (vif_flagged & l1_flagged) |
    (vif_flagged & corr_flagged)
) - interaction_terms
