...
# ============================================================
# PURPOSE: Exposes agent pipeline as REST API endpoints
# FLOW: HTTP request → endpoint → agent.run() → JSON response
# CONNECTED TO: attrition_agent.py (called here)
# ACCESS: localhost:8000/docs for Swagger UI
# ============================================================
...
# --- Step 1: Load config and init app ---
...
def health_check():
    """
    Verifies all components are loaded and ready.
    Check this first after starting server.
    """
    checks = {}
...
# ============================================================
# ENDPOINT 2: Explain single employee
# ============================================================
@app.get("/explain/{employee_id}")
def explain_employee(employee_id: str):
    """
    Full risk report for one employee.
    Returns P(attrition), SHAP drivers, LLM recommendation,
    compensation analysis, and policy references.
    """
    try:
...
# ============================================================
# ENDPOINT 3: Weekly digest
# ============================================================
@app.get("/report/weekly")
def weekly_digest():
    """
    Returns aggregated weekly digest of HIGH + CRITICAL employees.
    Shows unactioned alerts, tier distribution, budget summary.
    """
...
# ============================================================
# ENDPOINT 4: Department heatmap
# ============================================================
@app.get("/report/department/{dept_name}")
def department_report(dept_name: str):
    """
    Risk heatmap for a specific department.
    Shows avg P(attrition) by band, high risk count, budget needed.
    """
    scored_df = pd.read_csv("data/synthetic/scored_employees.csv")
...
# ============================================================
# ENDPOINT 5: Batch score CSV upload
# ============================================================
@app.post("/score/batch")
async def batch_score(file: UploadFile = File(...)):
    """
    Upload a CSV of employee IDs.
    Returns risk scores for all employees in file.
    Only scores employees already in scored_employees.csv.
    """
    ...
    # Merge uploaded IDs with pre-computed scores
...
# ============================================================
# ENDPOINT 6: Budget simulation
# ============================================================
@app.get("/report/budget-sim")
def budget_simulation(correction_type: str = "CRITICAL"):
    """
    Simulates budget impact of acting on a specific risk tier.
    Returns total cost, expected retentions, ROI.
    """
    scored_df = pd.read_csv("data/synthetic/scored_employees.csv")
    ...
    expected_retentions = (subset["p_attrition"] * 0.75).sum()  # 75% retention success rate
...
# ============================================================
# ENDPOINT 7: Audit log
# ============================================================
@app.get("/audit/recent")
def recent_audit(limit: int = 20):
    """
    Returns recent agent runs from SQLite audit log.
    Shows every score, recommendation, routing decision.
    """
    db_path = cfg["paths"]["audit_db"]

    if not Path(db_path).exists():
        return {"message": "No audit log yet — run agent first", "records": []}
